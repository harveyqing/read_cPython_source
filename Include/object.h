...
/* Python中一切对象的基石。
 * 每个PyObject中包含一个堆对象管理的_PyObject_HEAD_EXTRA,
 * 一个用于垃圾回收的引用计数（ob_refcnt），
 * 一个指向当前对象类型的类型指针(ob_type)
 */ 
...
/*
 * Python中变长（容器类）对象的基石。
 * 每个PyVarObject中包含一个PyObject（PyObject是一切变量的基石）变量，
 * 一个保存容器中元素个数的ob_size（注意是元素个数，而不是结构体的字节数）
 */
typedef struct {
    PyObject ob_base;
